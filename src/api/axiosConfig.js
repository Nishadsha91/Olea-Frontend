...
// const axiosInstance = axios.create({
//   baseURL: "http://184.73.8.191:8000/api", // ✅ direct backend API
// });
...
const axiosInstance = axios.create({
  baseURL: "https://oleainn.duckdns.org/api", // ✅ use your secure backend domain
});
...
axiosInstance.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (error.response && error.response.status === 401 && !originalRequest._retry) {
      ...
      if (refreshToken) {
        try {
          // ✅ use same domain here too
          const res = await axios.post("https://oleainn.duckdns.org/api/token/refresh/", {
            refresh: refreshToken,
          });
